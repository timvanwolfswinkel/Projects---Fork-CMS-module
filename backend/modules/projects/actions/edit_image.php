...
/**
 * This is the edit action, it will display a form to edit an existing project image.
 *
 * @author Bart De Clercq <info@lexxweb.be>
 */
class BackendProjectsEditImage extends BackendBaseActionEdit
{
	/**
	 * Execute the action
	 */
	...
	{
		$this->project = BackendProjectsModel::getImage($this->getParameter('project_id', 'int'));
	...
	protected function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				$this->redirect(BackendModel::createURLForAction('images') . '&project_id=' . $this->project['id'] . '&report=edited&var=' . urlencode($item['title']) . '&highlight=row-' . $item['id']);
			}
		}
	}
}
